...
func NewNodesStats(client *http.Client, url *url.URL) *NodesStats {
	return &NodesStats{
		client: client,
		url:    url,
		up:     common.UpMetric(namespace, "node_stats", "scrape of Elasticsearch node stats successful."),
...
func (c *NodesStats) getNodeMetric() (nodeStatsResponse, error) {
	...
	if res.StatusCode != http.StatusOK {
		return nsr, common.HTTPStatusError(res.StatusCode)
	}

	body, err := ioutil.ReadAll(res.Body)
	if err != nil {
		logger.Logger.Warningf("can not get node stats %s", err)
		return nsr, err
	}
	...
	for _, node := range nsr.Nodes {
		for _, metric := range c.nodeMetrics {
			ch <- prometheus.MustNewConstMetric(
				...
				metric.Type,
				metric.Value(node),
				metric.Labels(nsr.ClusterName, node)...,
			)
		}
		...
		for collector, gcStats := range node.JVM.GC.Collectors {
			for _, metric := range c.gcCollectionMetrics {
				ch <- prometheus.MustNewConstMetric(
					...
					metric.Labels(nsr.ClusterName, node, collector)...,
				)
			}
		}
		...
		for breaker, bstats := range node.Breakers {
			for _, metric := range c.breakerMetrics {
				ch <- prometheus.MustNewConstMetric(
					...
					metric.Type,
					metric.Value(bstats),
					metric.Labels(nsr.ClusterName, node, breaker)...,
				)
			}
		}
		...
		for pool, pstats := range node.ThreadPool {
			for _, metric := range c.threadPoolMetrics {
				ch <- prometheus.MustNewConstMetric(
					...
					metric.Type,
					metric.Value(pstats),
					metric.Labels(nsr.ClusterName, node, pool)...,
				)
			}
		}
		...
		for _, fsStats := range node.FS.Data {
			for _, metric := range c.filesystemMetrics {
				ch <- prometheus.MustNewConstMetric(
					...
					metric.Labels(nsr.ClusterName, node, fsStats.Mount, fsStats.Path)...,
				)
			}
		}
